...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const storedLang = localStorage.getItem('iz_admin_language') as Language;
    if (storedLang && (storedLang === 'en' || storedLang === 'tr')) {
      setLanguageState(storedLang);
      document.cookie = `iz_admin_language=${storedLang};path=/;max-age=31536000`;
    } else {
      // Auto detect from browser
      const browserLang = navigator.language.slice(0, 2).toLowerCase();
      const detected: Language = browserLang === 'tr' ? 'tr' : 'en';
      setLanguageState(detected);
      localStorage.setItem('iz_admin_language', detected);
      document.cookie = `iz_admin_language=${detected};path=/;max-age=31536000`;
    }
    setMounted(true);
  }, []);
  ...
  const setLanguage = (lang: Language) => {
    ...
    localStorage.setItem('iz_admin_language', lang);
    document.cookie = `iz_admin_language=${lang};path=/;max-age=31536000`;
